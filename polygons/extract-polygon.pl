#!/usr/bin/perl
#
# This Will Load a Drontlink Mission File ( ARGV[0] )
# The Result Is A KML File ( mission-polygon.kml )
# Extracting the bounding box from the mission file
#
...
my $template = HTML::Template->new(filename => 'placemark.tmpl');
$template->param(MINLAT => $min_lat);
$template->param(MAXLAT => $max_lat);
$template->param(MINLON => $min_lon);
$template->param(MAXLON => $max_lon);
...
$template->param(LAT1 => $outside_coords[0]->{'LAT'});
$template->param(LAT2 => $outside_coords[1]->{'LAT'});
$template->param(LAT3 => $outside_coords[2]->{'LAT'});
$template->param(LAT4 => $outside_coords[3]->{'LAT'});
$template->param(LON1 => $outside_coords[0]->{'LON'});
$template->param(LON2 => $outside_coords[1]->{'LON'});
$template->param(LON3 => $outside_coords[2]->{'LON'});
$template->param(LON4 => $outside_coords[3]->{'LON'});
